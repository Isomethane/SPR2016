function unitSkeleton() {
    ...
    this.init = function(anim) {
        ...
        anim.lightingManager.addPointLight(new pointLight(position, colors[1], colors[1]));
        for (var i = 1; i < 50; i++) {
            ...
            position.speed = Math.random() * 5;
            position.direction = Math.random() > 0.5 ? 1 : -1;
            anim.lightingManager.addPointLight(new pointLight(position, colors[i % 6], colors[i % 6]));
        }
